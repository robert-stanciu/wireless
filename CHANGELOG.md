...
You are reading the `1.x` line. A feature ships to both on the same day; the source difference
between them is `src/Lifecycle.php`,
...
### Fixed
- Livewire 3 keeps `hydrateForUpdate()` protected on `HandleComponents` (Livewire 4 moved it to
  `HandleSynths`), so `set()` borrows that scope rather than reimplementing the synth lookup — a
  typed property, a Carbon or a backed enum is hydrated on this line exactly as on the other.
