 ...
 * `call` gained the client metadata and the call index — see the 4.x line of this package. Every
 * argument is passed POSITIONALLY to every registered hook, so a wrong count is an
 * ArgumentCountError rather than a quietly skipped hook.
 *
 * @internal
 */
final class Lifecycle
{
    ...
    public static function updateProperties(Component $component, ComponentContext $context, array $values): void
    {
        ...
        // hydrateForUpdate is what turns '2025-01-01' into a Carbon and 'live' into a backed enum;
        // without it a typed property assignment aborts with a bare 419. Livewire 3 keeps it
        // protected on HandleComponents (Livewire 4 moved it to HandleSynths and made it public),
        // and its own update pipeline is the only caller — so this cycle borrows the scope rather
        // than reimplementing the synth lookup.
        $hydrate = Closure::bind(
            fn (string $path, mixed $value): mixed => $this->hydrateForUpdate([], $path, $value, $context),
            $handler,
            HandleComponents::class,
        );

        foreach ($values as $path => $value) {
            $finish = $handler->updateProperty($component, $path, $hydrate($path, $value), $context);
